...
{
    (void)u;
    console_printf("%s  size=%u cap=%u\n", f->name, f->size, f->cap);
}

int32_t rdisk_cmd_dir(int32_t argc, char **argv)
{
    if (!g_disk_inited) {
        console_printf("Disk not initialized. Please format first.\n");
        return -1;
    }
...
int32_t rdisk_cmd_type(int32_t argc, char **argv)
{
    ...
    len = strlen_s(argv[1]);
    if (len >= RDISK_NAME_MAX) {
        console_printf("Filename too long.\n");
        return 0;
    }
...
int32_t rdisk_cmd_loadhex(int32_t argc, char **argv)
{
    ...
    if (cap == 0){
        console_printf("caphex must be >0\n");
        return -1;
    }
    if (!g_disk_inited) {
        console_printf("Disk not initialized. Please format first.\n");
    ...
    if (rdisk_create(&g_disk, argv[1], cap) != RDISK_OK){
        console_printf("(create failed) maybe exists? try del first.\n");
        return -1;
    }
    ...
    if (rdisk_get_ptr_cap(&g_disk, argv[1], &ptr, &real_cap) != RDISK_OK){
        console_printf("(get ptr failed)\n");
        return -1;
    }

    console_printf("[LOADHEX] send Intel HEX lines, end with EOF(:..01..)\n");
    ...
    console_printf("(IHEX)> ");
    while (1){
        int n = console_readline(ihex, sizeof(ihex), 0xFFFFFFFFu);
        if (n <= 0) continue;

        r = ihex_feed_line_to_buffer(ihex, dst, &len, real_cap);
        if (r == 99) break;
        if (r < 0){
            console_printf("LOADHEX ERROR: %d\n", r);
            break;
        }
        console_printf("(IHEX)> ");
    }
